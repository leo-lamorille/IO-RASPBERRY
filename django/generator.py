...
class Sensor:
    ...
    @staticmethod
    def columns():
        return "dashboard_sensor(mac, name, interval)"
...
TIMELAPSE= 2 * 365 * 86400 # 10 * 86400 = 10 days
...
sensors = [Sensor('24:6F:28:24:6B:50', 'Cécile En Cieux', randint(5, 100)), 
           Sensor('24:6F:28:24:6B:51', 'Gérard Menfaim', randint(5, 100)), 
           Sensor('24:6F:28:24:6B:52', 'Gérard Manvussa', randint(5, 100)), 
           Sensor('24:6F:28:24:6B:53', 'Gérard Menjoui', randint(5, 100)), 
           Sensor('24:6F:28:24:6B:54', 'Tom Égérie', randint(5, 100)),
           Sensor('24:6F:28:24:6B:55', 'Yves Atrovite', randint(5, 100)),
           Sensor('24:6F:28:24:6B:55', 'Yvon Enbavé', randint(5, 100)),
           Sensor('24:6F:28:24:6B:55', 'Yvon Tremblay', randint(5, 100))]
datas = []

SENSOR_BUFFER = "INSERT INTO {} VALUES ".format(Sensor.columns())
DATAS_BUFFER = "INSERT INTO {} VALUES ".format(Data.columns())


cpt = 0
for sensor in sensors:
    print("{}/{} sensors done".format(cpt, len(sensors)))
    cpt+=1
    SENSOR_BUFFER += "\n\t{},".format(str(sensor))
    last_humidity = 50
